...
__global__ void adam_w_kernel(float* __restrict__ values,
    ...
    // weight decay
    values[idv] -= weight_decay * values[idv];
}
...
void adam_w(      data::DenseMatrix<float>& values,
          ...
          float max,
          float lasso,
          float ridge, float weight_decay) {
    // clang-format on
    ...
    if (data::is_gpu(DEV)) {
        ...
        adam_w_kernel<<<grid, block>>>(values.first<DEV>(),
                                       gradients.first<DEV>(),
