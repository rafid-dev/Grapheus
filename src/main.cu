    ...
    void train(dataset::BatchLoader<chess::Position>& loader,
        ...
        for (int i = epoch_continuation; i < epochs; i++) {
            ...
            for (int b = 0; b < epoch_size / loader.batch_size; b++) {
                ...
                t.stop();
                if (b > 0 && (b == (epoch_size / loader.batch_size) - 1 || t.elapsed() - prev_duration > 1000)) {
                    prev_duration = t.elapsed();

                    printf("\rep/ba = [%3d/%5d], ", i, b);
                    printf("batch_loss = [%1.8f], ", batch_loss);
                    printf("epoch_loss = [%1.8f], ", epoch_loss / b);
                    printf("speed = [%9d pos/s], ", (int) round(1000.0f * loader.batch_size * b / t.elapsed()));
...
struct RiceModel : ChessModel {
    ...
    RiceModel()
        ...
        auto ft = add<FeatureTransformer>(in1, in2, 256);
        auto re = add<ReLU>(ft);
        ...
        set_file_output("../result/testnet");
    ...
    inline int index(chess::Square piece_square, chess::Piece piece, chess::Square king_square, chess::Color view) {
        ...
        const int index = relative_piece_square + piece_type * PIECE_TYPE_FACTOR + (piece_color == view) * PIECE_COLOR_FACTOR
...
struct ChessModelBinpack : nn::Model {
    ...
    // seting inputs
    virtual void setup_inputs_and_outputs(std::vector<binpack::binpack::TrainingDataEntry>& positions) = 0;

    // train function
    void train(carbon_dataloader::DataLoader& loader,
               int                            epochs             = 1000,
               int                            epoch_size         = 1e7,
               float                          start_lambda       = 1.0,
               float                          end_lambda         = 0.6,
...
struct RiceModelBinpack : ChessModelBinpack {
    SparseInput* in1;
    SparseInput* in2;
    ...
    inline int index(uint8_t pieceType, uint8_t pieceColor, int square, uint8_t view, int kingSquare) {
    ...
    void setup_inputs_and_outputs(std::vector<binpack::binpack::TrainingDataEntry>& positions) {
        in1->sparse_output.clear();
        in2->sparse_output.clear();
...
int main(int argc, const char* argv[]) {
    ...
    // for (auto& file : std::filesystem::recursive_directory_iterator(R"(/mnt/f/trainingdata/data)")) {
    ...
    // model.train(loader, 500, 1e8, 1.0f, 0.6f, 0);
